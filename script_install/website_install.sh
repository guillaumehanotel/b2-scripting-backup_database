...
if [ ! -d /var/www/html/appli_web/ ]; then
	# Import du site web
	git clone https://github.com/guillaumehanotel/appli_web $HOME/appli_web/

	# Déplacement du site dans le serveur
	sudo mv $HOME/appli_web/ /var/www/html

	# Mettre l'utilisateur dans le groupe www-data
	sudo usermod -g www-data $USER

	# Change user & group of website
	sudo chown -R www-data:www-data /var/www/html/appli_web/
fi
...
if [ ! -f /etc/apache2/sites-available/001-appli_web.conf ]; then
	...
	sudo cp $HOME/postgreSQL/script_install/001-appli_web.conf /etc/apache2/sites-available/

	# Activation du virtual host
	sudo a2ensite 001-appli_web
